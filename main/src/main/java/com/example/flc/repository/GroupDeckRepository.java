...
public interface GroupDeckRepository extends JpaRepository<GroupDeck, Long> {
        ...
        @Query("""
                        ...
                        AND deck.status = true
                                """)
        List<GroupDeck> findByGroupDeckStatus(
                        @Param("groupId") Long groupId,
                        @Param("status") String status);

        // @Query("SELECT gd FROM GroupDeck gd JOIN GroupMember gm ON gd.group.id =
        // gm.group.id " +
        // "WHERE gd.group.id = :groupId AND gd.status = :status AND gm.user.id =
        // :userId")
