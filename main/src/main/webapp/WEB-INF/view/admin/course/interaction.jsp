<%@page contentType="text/html" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        ...
        <body>
            <header class="top-nav" style="height: 76.4px;">
                <div class="brand">
                        ...
                        <label>
                            ...
                            <select name="templateKey" id="templateKey" required>
                                <c:forEach var="template" items="${templates}">
                                    <option value="${template.key}" ${config.templateKey==template.key ? 'selected' : ''
                                        }>
                    ...
                    <div class="task-list">
                        <c:forEach var="task" items="${tasks}" varStatus="status">
                            <article class="task-card">
                                ...
                                <form class="task-form" action="/admin/course/${deck.id}/interaction/tasks"
                                    method="post">
                                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
                                    ...
                                    <label>
                                        ...
                                        <select name="targetCardId" required>
                                            ...
                                            <c:forEach var="card" items="${cards}">
                                                <option value="${card.id}" ${task.targetCard.id==card.id ? 'selected'
                                                    : '' }>
                                                    ${card.word}
                                                </option>
                                            </c:forEach>
                                        ...
                                        <label>
                                            ...
                                            <select name="templateVariant" required>
                                                ...
                                                <c:forEach var="variant" items="${variants}">
                                                    <option value="${variant.key}" ${task.templateVariant==variant.key
                                                        ? 'selected' : '' }>
                                                        ${variant.displayName}
                                                    </option>
                                                </c:forEach>
